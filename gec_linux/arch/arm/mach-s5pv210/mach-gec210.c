...
extern struct s3cfb_lcd *gec210_get_lcd(void);
...
static struct s3c2410_uartcfg gec210_uartcfgs[] __initdata = {
	{
		.hwport		= 0,
		.flags		= 0,
...
static struct s5p_media_device gec210_media_devs[] = {
	[0] = {
		.id = S5P_MDEV_MFC,
		.name = "mfc",
		.bank = 0,
...
static void gec210_fixup_bootmem(int id, unsigned int size) {
	int i;

	for (i = 0; i < ARRAY_SIZE(gec210_media_devs); i++) {
		if (gec210_media_devs[i].id == id) {
			gec210_media_devs[i].memsize = size;
		}
	}
}
...
static void __init gec210_setup_clocks(void)
{
	struct clk *pclk;
	struct clk *clk;
...
static void __init gec210_wifi_init(void)
{
...
static int tvp5150_power_en(int onoff)
{
	...
	if (err)
		printk(KERN_ERR "#### failed to request GPJ3_1 for RESET\n");	

	s3c_gpio_setpull(S5PV210_GPJ3(1), S3C_GPIO_PULL_NONE);
	gpio_direction_output(S5PV210_GPJ3(1), 0);//\B8\B4λ
	mdelay(2);
	gpio_direction_output(S5PV210_GPJ3(1), 1);//\D5\FD\B3\A3
	gpio_free(S5PV210_GPJ3(1));

#if 0
        err = gpio_request(S5PV210_GPE1(2), "GPE1_2");
	if (err)
		printk(KERN_ERR "#### failed to request GPE1_2 for RESET\n");	

	s3c_gpio_setpull(S5PV210_GPE1(2), S3C_GPIO_PULL_NONE);
...
void s3c6410_mcp2510_init(void)
{
	/* EINT0 FOR MACP2510 EINT*/
	s3c_gpio_cfgpin(S5PV210_GPH0(3), S3C_GPIO_SFN(0xf));	
#if 0
	if (!gpio_request(S5PV210_GPH0(3), "EINT3")) {
		s3c_gpio_cfgpin(S5PV210_GPH0(3), S3C_GPIO_SFN(1));
...
static struct mcp251x_platform_data mcp251x_data = 
{
	...
	.board_specific_setup = &s3c6410_mcp2510_init,
...
//XJM
 static void smdk_m25p10_cs_set_level(int high)    //spi\BF\D8\D6\C6\C6\F7\BB\E1\D3\C3\D5\E2\B8\F6\B7\BD\B7\A8\C9\E8\D6\C3cs
{
        u32 val;
        val = readl(S5PV210_GPBDAT);
        if (high)
                val |= (1<<3);
        else
                val &= ~(1<<3);
        writel(val, S5PV210_GPBDAT);
}
...
#if 0
	[1] = {
		.modalias        = "mcp2510",	/* device node name */
		.mode            = SPI_MODE_0,	/* CPOL=0, CPHA=0 */
		.max_speed_hz    = 20000000,
		
		.bus_num         = 1,       //\B6\D4Ӧ\D5\E2\B8\F6SPI\C9豸\B9\D2\D4\DAϵͳ\B5\C4\C4ĸ\F6SPI\BF\D8\D6\C6\C6\F7\C9\CF
		.irq             = IRQ_EINT3,
...
//		.modalias        = "m25p10a",	/* device node name */
		.mode            = SPI_MODE_0,	/* CPOL=0, CPHA=0 */
		.max_speed_hz    = 20000000,
	...
	[0] = {
		...
		.bus_num         = 0,       //\B6\D4Ӧ\D5\E2\B8\F6SPI\C9豸\B9\D2\D4\DAϵͳ\B5\C4\C4ĸ\F6SPI\BF\D8\D6\C6\C6\F7\C9\CF
		.irq             = IRQ_SPI0,
		.chip_select     = 0,
...
{
//S5PV210_GPH1(7) 对应EXT_INT15 ， S3C_GPIO_SFN(0XF)对应中断模式
	printk("config GPH1-6 to EINT14\n");
	if( gpio_request(S5PV210_GPH1(6), "GPH1_7") )
...
//用来判断笔状态，当有触摸的时候，触摸屏将会产生一个从高到低的电平变化，主机将采集到0
...
#ifdef CONFIG_TOUCHSCREEN_FT5X06
	{
		I2C_BOARD_INFO("ft5x06-iic", (0x70 >> 1)),
...
static struct platform_device *gec210_devices[] __initdata = {
...
static void __init gec210_map_io(void)
{
	struct s3cfb_lcd *lcd = gec210_get_lcd();
	int fimd_size;

	s5p_init_io(NULL, 0, S5P_VA_CHIPID);
	s3c24xx_init_clocks(24000000);
	s5pv210_gpiolib_init();
	s3c24xx_init_uarts(gec210_uartcfgs, ARRAY_SIZE(gec210_uartcfgs));

	fimd_size = PXL2FIMD(lcd->width * lcd->height);
	if (fimd_size != S5PV210_VIDEO_SAMSUNG_MEMSIZE_FIMD) {
		gec210_fixup_bootmem(S5P_MDEV_FIMD, fimd_size);
	}
	s5p_reserve_bootmem(gec210_media_devs, ARRAY_SIZE(gec210_media_devs));
...
static void __init gec210_fixup(struct machine_desc *desc,
...
static void __init gec210_machine_init(void)
{
	int ret;
	arm_pm_restart = smdkc110_pm_restart;

	s3c_usb_set_serial();
	platform_add_devices(gec210_devices, ARRAY_SIZE(gec210_devices));
#ifdef CONFIG_ANDROID_PMEM
	...
	{
		struct s3cfb_lcd *mlcd = gec210_get_lcd();
		if (!(mlcd->args & 0x0f)) {
			if (readl(S5PV210_GPF0_BASE + 0x184) & 0x10)
				mlcd->args |= (1 << 7);
		}
		gec210_fb_data.lcd = mlcd;
	...
	if (!gpio_request(S5PV210_GPB(5), "SPI_CS1")) {
		...
		s5pv210_spi_set_info(1, S5PV210_SPI_SRCCLK_PCLK,
			ARRAY_SIZE(smdk_spi1_csi));
		printk("====gpio_direction_output(S5PV210_GPB(5)=%d)\n",gpio_get_value(S5PV210_GPB(5)));
	}
	spi_register_board_info(s3c_spi_devs, ARRAY_SIZE(s3c_spi_devs));
#endif
