...
static int gec210_hw_params(struct snd_pcm_substream *substream,
	struct snd_pcm_hw_params *params)
{
	...
	case 64000:
	case 96000:
		clk_set_rate(clk_out, rfs*params_rate(params)); // rfs*params_rate(params)   12288000
	...
	case 88200:
	default:
		clk_set_rate(clk_out, rfs*params_rate(params));//11289600 
	...
	printk("GEC210  PSR=%d RFS=%d BFS=%d\n", psr, rfs, bfs);
...
/*
 * GEC210 WM8960 DAI operations.
 */
static struct snd_soc_ops gec210_ops = {
	.hw_params = gec210_hw_params,
};

/* GEC210 Playback widgets */
...
static int gec210_wm8960_init_paiftx(struct snd_soc_codec *codec)
{
	int ret;

	/* Add gec210 specific Capture widgets */
...
static int gec210_wm8960_init_paifrx(struct snd_soc_codec *codec)
{
	int ret;

	/* Add gec210 specific Playback widgets */
...
static struct snd_soc_dai_link gec210_dai[] = {
{ /* Primary Playback i/f */
	.name = "WM8960 PAIF RX",
	.stream_name = "Playback",
	.cpu_dai = &s3c64xx_i2s_dai[I2S_NUM],
	.codec_dai = &wm8960_dai,
	.init = gec210_wm8960_init_paifrx,
	.ops = &gec210_ops,
},
{ /* Primary Capture i/f */
	.name = "WM8960 PAIF TX",
	...
	.cpu_dai = &s3c64xx_i2s_dai[I2S_NUM],
	.codec_dai = &wm8960_dai,
	.init = gec210_wm8960_init_paiftx,
	.ops = &gec210_ops,
},
};

static struct snd_soc_card gec210 = {
	.name = "gec",
	.platform = &s3c_dma_wrapper,
	.dai_link = gec210_dai,
	.num_links = ARRAY_SIZE(gec210_dai),
};

static struct snd_soc_device gec210_snd_devdata = {
	.card = &gec210,
	.codec_dev = &soc_codec_dev_wm8960,
};

static struct platform_device *gec210_snd_device;
static int __init gec210_audio_init(void)
{
	int ret;

	gec210_snd_device = platform_device_alloc("soc-audio", 0);
	if (!gec210_snd_device)
		return -ENOMEM;

	platform_set_drvdata(gec210_snd_device, &gec210_snd_devdata);
	gec210_snd_devdata.dev = &gec210_snd_device->dev;
	ret = platform_device_add(gec210_snd_device);

	if (ret)
		platform_device_put(gec210_snd_device);

	return ret;
}

static void __exit gec210_audio_exit(void)
{
	debug_msg("%s\n", __func__);

	platform_device_unregister(gec210_snd_device);
}
module_init(gec210_audio_init);
module_exit(gec210_audio_exit);

MODULE_AUTHOR("Pyeongjeong Lee, leepjung@crz-tech.com");
MODULE_DESCRIPTION("ALSA SoC GEC210 WM8960");
MODULE_LICENSE("GPL");
