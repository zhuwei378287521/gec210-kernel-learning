...
// by xjm
//Bit[8] terminal shutdown enable 0 disable
//Bit[7:6] analogue bias optimisation
//Bit[5] reserved
//Bit[4] DAC Mono Mix 0->stereo 1->mono
//Bit[3:2] ADC Data output Select  defaul 00
//Bit[1] Slow clock select default 0
//Bit[0] enable shlow clock select (used for volume update timeout and jack detect debounce default 0 disabled
	reg = snd_soc_read(codec, WM8960_ADDCTL1);	// 17
	snd_soc_write(codec, WM8960_ADDCTL1, reg | 0x001);

//Bit[8:7] reserved 
//Bit[6] headphone switch enable  default 0 disable(headphone jack detect
//Bit[5] headphone switch polarity 0 high for HP 1 high for SP
//Bit[4] reserved
/*Bit[3] Tristates ADCDAT and switches ADCLRC DACLRC and BCLK to inputs
 0 = ADCDAT is an output; ADCLRC, DACLRC and BCLK are inputs (slave mode) or outputs (master mode) 
1 = ADCDAT is tristated; DACLRC and BCLK are inputs; ADCLRC is an input (when not configured as a GPIO) 
 */
 /*Bit[2] Selects disable mode for ADCLRC and DACLRC (Master mode) 0 = ADCLRC disabled when ADC (Left and 
Right) disabled; DACLRC disabled when DAC (Left and Right) disabled. 1 = ADCLRC and DACLRC disabled only when 
ADC (Left and Right) and DAC (Left and Right) are disabled. 
 */
	...
	snd_soc_write(codec, WM8960_LOUT1, reg | 0x070);
	reg = snd_soc_read(codec, WM8960_ROUT1);	// 03
	snd_soc_write(codec, WM8960_ROUT1, reg | 0x070);
	...
	snd_soc_write(codec, WM8960_LINVOL, 0x0113);
	// IPVU=1, RINMUTE=0, RIZC=0, RINVOL=01_0111
	snd_soc_write(codec, WM8960_RINVOL, 0x0113);
