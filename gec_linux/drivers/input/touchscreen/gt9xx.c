...
#if MULTI_TOUCH //////////////////////for multi_touch//////////XJM
...
static void goodix_ts_work_func(struct work_struct *work)
{
    ...
    if (touch_num)
    {
        ...
        #endif
         //   printk("xjm======touch_num=%d\n",touch_num);
            gtp_touch_down(ts, id, input_x+5, input_y+3, input_w);
        }
    }
    else if (pre_touch)
    {
...
static s8 gtp_request_irq(struct goodix_ts_data *ts)
{
    ...
    ret  = request_irq(ts->client->irq, 
                       ...
                       irq_table[ts->int_trigger_type],/* IRQ_TYPE_LEVEL_HIGH   IRQ_TYPE_LEVEL_HIGH  IRQ_TYPE_EDGE_BOTH irq_table[ts->int_trigger_type],   
                        */
                       ts->client->name,
                       ts);
    if (ret)
...
static s8 gtp_request_input_dev(struct goodix_ts_data *ts)
{
...
#if MULTI_TOUCH  //FOR MULIT TOUCH  ///////////////
    ...
    input_set_abs_params(ts->input_dev, ABS_MT_TRACKING_ID, 0, 255, 0, 0); // xjm mask
    input_set_abs_params(ts->input_dev, ABS_PRESSURE, 0, 255, 0, 0); //xjm
...
////////////////////////////xjmmmmmmmm//////////////////////////////////////
    ...
    input_set_abs_params(ts->input_dev, ABS_X, 0, ts->abs_x_max, 0, 0);  //xjm
 ...
 static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    printk("Clock ticks for an esd cycle: %d", ts->clk_tick_cnt);  
