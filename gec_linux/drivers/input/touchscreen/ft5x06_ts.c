...
//#define I2C_CTPM_ADDRESS       0x38	//lqm test
...
{
#include "IT_android_V11_20131230_app.i"	//lqm masked for test.
};
...
int fts_ctpm_auto_upg(void)
{
    ...
    if ( uc_tp_fm_ver == 0xa6  ||   //the firmware in touch panel maybe corrupted   //p76ti:fm_ver=0x13 default:0x6
...
static int ft5x0x_read_data(void)
{
	...
	event->au16_x[i] = (s16)(buf[5 + 6*i] & 0x0F)<<8 | (s16)buf[6 + 6*i];
        event->au16_y[i] = (s16)(buf[3 + 6*i] & 0x0F)<<8 | (s16)buf[4 + 6*i];
#endif
...
int ft5x0x_touch_key_process(struct input_dev *dev, int x, int y, int touch_event)
{
    ...
    else if ( y < 90 &&y > 60)//73
    {
        key_id = 0;
    }
    
    else if ( y < 60 && y > 30)//43
    {
        key_id = 1;
    }
    else if (y < 30)//0
...
static void ft5x0x_report_value(void)
{
    ...
    for (i  = 0; i < event->touch_point; i++)
    {
	//tmp=event->au16_y[i];event->au16_y[i]=event->au16_x[i];event->au16_x[i]=tmp;
	//printk("p[%d]=[%d,%d]\r\n",i,event->au16_x[i],event->au16_y[i]);//lqm added.
        if (event->au16_x[i] < /*SCREEN_MAX_X*/800 && event->au16_y[i] < /*SCREEN_MAX_Y*/480)
        // LCD view area
        {
	    //printk("p[%d]=[%d,%d]\r\n",i,event->au16_x[i],event->au16_y[i]);//lqm added.
#ifdef CFG_ONBOARD_FT5X06
	    event->au16_y[i] = 480 - event->au16_y[i];
#endif
        ...
        {
#if CFG_SUPPORT_TOUCH_KEY
            if (event->au16_x[i] >= /*SCREEN_MAX_X*/800)
            {
                //printk("key x = %d\n", event->au16_y[i]);
...
//for single xjm
	...
	//		printk("x=%d y=%d point=%d\n",event->au16_x[i],event->au16_y[i],event->touch_point);
...
static irqreturn_t ft5x0x_ts_interrupt(int irq, void *dev_id)
{
    ...
    disable_irq_nosync(IRQ_EINT14);//lqm changed
...
ft5x0x_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	set_irq_type(IRQ_EINT14, IRQF_TRIGGER_FALLING);

    err = request_irq(IRQ_EINT14, ft5x0x_ts_interrupt, IRQF_TRIGGER_FALLING, "ft5x0x_ts", ft5x0x_ts);
    if (err < 0)
    {
