...
//#define FT5X06_I2C_ADDRESS	(0x3d)
...
{
//#include "gec210_app.h"
#include "IT_android_V11_20131230_app.i"
};
...
static FTS_BOOL cmd_write(struct i2c_client * client, FTS_BYTE btcmd,FTS_BYTE btPara1,FTS_BYTE btPara2,FTS_BYTE btPara3,FTS_BYTE num)
{
    ...
    write_cmd[2] = btPara2;
    write_cmd[3] = btPara3;
    return i2c_write_interface(client, client->addr, write_cmd, num);
}

static FTS_BOOL byte_write(struct i2c_client * client, FTS_BYTE* pbt_buf, FTS_DWRD dw_len)
{
    return i2c_write_interface(client, client->addr, pbt_buf, dw_len);
}

static FTS_BOOL byte_read(struct i2c_client * client, FTS_BYTE* pbt_buf, FTS_BYTE bt_len)
{
    return i2c_read_interface(client, client->addr, pbt_buf, bt_len);
}
...
#if MULTI_TOUCH  // xjm   multi touch
	for(i = 0; i < 5; i++)
	{
		...
		if(id >= 0 && id <= 4)
		{
			...
			{
				input_report_abs(ts->input_dev, ABS_MT_TOUCH_MAJOR, 255); //xjm
				input_report_abs(ts->input_dev, ABS_MT_POSITION_X, ts->node[id].x );
				input_report_abs(ts->input_dev, ABS_MT_POSITION_Y, ts->node[id].y-10 );//ÐÞÕý±ßÔµ´¥Ãþ
			...
			//	 printk("[%d]down: x = %4d, y = %4d\n", id, ts->node[id].x, ts->node[id].y-10);
			}
			else if(event == 0x01)
			{
				input_report_abs(ts->input_dev, ABS_MT_TOUCH_MAJOR, 0);
				input_report_abs(ts->input_dev, ABS_MT_POSITION_X, ts->node[id].x );
				input_report_abs(ts->input_dev, ABS_MT_POSITION_Y, ts->node[id].y -10);
...
static int ft5x06_iic_probe(struct i2c_client * client, const struct i2c_device_id * id)
{
...
#if MULTI_TOUCH  //for multitouch   xjm
	...
	client->irq = IRQ_EINT14;

	/* config interrupt pin  (must support BTN_TOUCH or BTN_LEFT
events) */
